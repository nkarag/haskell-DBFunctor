...
### Main Features
 ...
 - Easy manipulation of your "tabular data" (e.g.,CSV files). Create arbitrary complex  data transformation flows for your tabular data with ease, in the functional way haskellers love, without the need of a database.
...
* **Data.CSV**:  It implements a CSV data type that can be converted to an RTable. It is based on the [Cassava](https://github.com/haskell-hvr/cassava) library.
 ...
 In this example, we will load a CSV file, turn it into an RTable and then issue a very simple query on it, just to show the whole concept. 
...
myJulExpression =
	...
	:-> (EtlR $   -- this is a series of Relational Algebra Operations
	     ROpStart
	  :. (Rel Operation 1) -- relational algebra operation
	  :. (Rel Operation 2))
...
FilterBy (\t ->	let fstChar = Data.Text.take 1 $ fromJust $ toText (t <!> "TABLE_NAME") 
				in fstChar == (pack "B"))
```
The second relational operation is a simple Projection expressed with the `
```
(Select ["OWNER", "TABLE_NAME"] $
	From Previous)
```
...
import  RTable.Data.CSV     (CSV, readCSV, csvToRTable)
import  Etl.Julius          
